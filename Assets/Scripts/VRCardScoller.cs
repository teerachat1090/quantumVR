...
public class VRCardScroller : MonoBehaviour
{
    ...
    void InitializeCards()
    {
        for (int i = 0; i < cards.Count; i++)
        {
            if (cards[i].cardObject != null)
            {
                ...
                cards[i].canvas = cards[i].cardObject.GetComponent<Canvas>();
                if (cards[i].canvas == null)
                {
                    cards[i].canvas = cards[i].cardObject.AddComponent<Canvas>();
                    cards[i].canvas.overrideSorting = true;
                }
    ...
    void UpdateCardVisuals()
    {
        for (int i = 0; i < cards.Count; i++)
        {
            ...
            if (isSelected)
            {
                cards[i].cardObject.transform.SetAsLastSibling();
                if (cards[i].canvas != null)
                {
                    cards[i].canvas.sortingOrder = 10;
                }
            }
            else
            {
